...
class Aplicacion:
    def __init__(self, root):
        ...
        self.seleccion_activa = False  # No se usa, pero la dejo por si acaso
        ...
        self.mapa_dialog = None  # <-- Inicialización correcta.
        ...
        self.entry_id.config(state="disabled")  # Deshabilitado por defecto
        ...
        # --- Botones ---
        # Frame para los botones, para que queden en una fila y lo centramos
        ...
        # --- Usar un Treeview para mostrar los datos en formato de tabla ---
        ...
        # Configurar los encabezados de las columnas correctamente
        ...
        self.tree.configure(selectmode="browse")  # Permitir seleccionar una sola fila a la vez
        ...
        self.tree.heading("Capacidad", text="Capacidad (kg)")  # <-- Añadido
        self.tree.heading("Carga Actual", text="Carga Actual")  # <-- Añadido
        self.tree.heading("Eficiencia", text="Eficiencia")
        ...
        self.tree.column("Distancia", width=70)  # <-- Ajustado
        self.tree.column("Partida", width=130)  # <-- Ajustado
        self.tree.column("Destino", width=130)  # <-- Ajustado
        self.tree.column("Capacidad", width=70)  # <-- Añadido
        self.tree.column("Carga Actual", width=70)  # <-- Añadido

        # Configuración para que la tabla y los botones se expandan con la ventana.
        self.root.grid_rowconfigure(8, weight=1)  # Fila del Treeview se expande verticalmente
        self.root.grid_columnconfigure(0, weight=1)  # Todas las columnas
        self.root.grid_columnconfigure(1, weight=1)  # se expanden
        self.root.grid_columnconfigure(2, weight=1)  # horizontalmente
        self.root.grid_columnconfigure(3, weight=1)  # de manera
        self.root.grid_columnconfigure(4, weight=1)  # uniforme.


        # Vincular el evento de selección a la función cargar_ruta_seleccionada
        self.tree.bind("<<TreeviewSelect>>", self.seleccionar_ruta)
        self.root.bind("<Button-1>", lambda event: self.tree.selection_remove(self.tree.selection()) \
                if event.widget not in (self.tree, self.btn_agregar, self.btn_buscar, self.btn_eliminar, \
                                        ...
                                        self.entry_partida, self.entry_destino) else None)  # <-- Sin btn_calcular_ruta
        self.root.bind("<Escape>", lambda event: self.limpiar_campos())  # <-- Para limpiar con Escape
    ...
    def buscar_ruta(self):
        ...
        if not id_ruta:  # Verificar si el campo ID está vacío
            ...
            self.entry_id.config(state="normal")  # Asegurar que el campo ID esté habilitado
