...
class Aplicacion:
    ...
    def limpiar_campos(self, event=None):
        ...
        print("DEBUG: Limpiando campos...")

        # *Siempre* habilitar el campo ID antes de limpiar
        self.entry_id.config(state="normal")
        print("DEBUG: limpiar_campos - ID habilitado (antes de limpiar)")
        ...
        # *Después* de limpiar, si NO hay selección, deshabilitar.
        if event:  # Si se llamó desde un evento (Escape)
            print("DEBUG: limpiar_campos - Llamada desde evento, deshabilitando ID")
            self.entry_id.config(state="disabled")
        else:
            print("DEBUG: limpiar_campos - No hay evento, el ID se mantiene habilitado")
        ...
        if os.path.exists(ruta_archivo):
            try:
                with open(ruta_archivo, "r", newline="", encoding="utf-8") as f:
                    ...
                    self.actualizar_lista()  # Mostrar los datos cargados en la tabla
                    # self.guardar_datos()  # Ya no guardar aqui, se carga al modificar

            except Exception as e:
                print(f"DEBUG: cargar_datos_iniciales - Error inesperado al cargar datos: {e}")
                self.insertar_datos_ejemplo()
                self.guardar_datos() # Guardar los datos
